...
export class KsMapPreview extends Component {
    ...
    _Ks_render() {
        ...
        var graphTextElements = self.mapContainerRef.el.querySelectorAll("div.graph_text");
            ...
            if(rec.ks_data_calculation_type !== "query"){
                if (rec.ks_model_id) {
                    if (rec.ks_chart_groupby_type == 'date_type' && !rec.ks_chart_date_groupby) {
                        self.mapContainerRef.el?.appendChild(this.createGraphTextElement("Select Group by date to create chart based on date groupby"));
                    } else if (rec.ks_chart_data_count_type === "count" && !rec.ks_chart_relation_groupby) {
                        self.mapContainerRef.el?.appendChild(this.createGraphTextElement("Select Group By to create chart view"));
                    } else if (rec.ks_chart_data_count_type !== "count" && (rec.ks_chart_measure_field.count === 0 || !rec.ks_chart_relation_groupby)) {
                        self.mapContainerRef.el?.appendChild(this.createGraphTextElement("Select Measure and Group By to create chart view"));
                    } else if (!rec.ks_chart_data_count_type) {
                        self.mapContainerRef.el?.appendChild(this.createGraphTextElement("Select Chart Data Count Type"));
                    } else {
                        this.get_map_data(rec);
                    }
                } else {
                    self.mapContainerRef.el?.appendChild(this.createGraphTextElement("Select a Model first."));
                }
            }else if(rec.ks_data_calculation_type === "query" && rec.ks_query_result) {
                if(rec.ks_xlabels && rec.ks_ylabels){
                        this.get_map_data(rec);
                } else {
                    self.mapContainerRef.el?.appendChild(this.createGraphTextElement("Please choose the X-labels and Y-labels"));
                }
            }else {
                    self.mapContainerRef.el?.appendChild(this.createGraphTextElement("Please run the appropriate Query"));
            }
        }
    ...
    async get_map_data(rec){
       if (this.mapContainerRef.el.querySelectorAll(".graph_text").length) {
            this.mapContainerRef.el?.querySelectorAll(".graph_text").forEach(function(el) {
                el.remove();
            });
       }
        var ks_data=[];
...
//        // Add zoom control
        ...
        chart.appear(1000, 100);
            ...
            this.mapContainerRef.el.innerHTML += renderToString("ksNoItemChartView", {});
}
}
//    }else{
