            ...
            if (rec.ks_data_calculation_type !== "query") {
                if (rec.ks_model_id) {
                    if (rec.ks_chart_groupby_type === 'date_type' && !rec.ks_chart_date_groupby) {
                        self.graphref.el.appendChild(this.createGraphTextElement("Select Group by date to create chart based on date groupby"));
                    } else if (rec.ks_dashboard_item_type !== 'ks_scatter_chart' && rec.ks_chart_data_count_type === "count" && !rec.ks_chart_relation_groupby) {
                        self.graphref.el.appendChild(this.createGraphTextElement("Select Group By to create chart view"));
                    } else if (rec.ks_dashboard_item_type !== 'ks_scatter_chart' && rec.ks_chart_data_count_type !== "count" &&
                               (rec.ks_chart_measure_field.count === 0 || !rec.ks_chart_relation_groupby)) {
                        self.graphref.el.appendChild(this.createGraphTextElement("Select Measure and Group By to create chart view"));
                    } else if (rec.ks_dashboard_item_type !== 'ks_scatter_chart' && !rec.ks_chart_data_count_type) {
                        self.graphref.el.appendChild(this.createGraphTextElement("Select Chart Data Count Type"));
                    } else if (rec.ks_dashboard_item_type === "ks_scatter_chart") {
                        if (rec.ks_scatter_measure_x_id && rec.ks_chart_measure_field) {
                            this.get_chart(rec);
                        } else {
                            self.graphref.el.appendChild(this.createGraphTextElement("Please Choose Measures"));
                        }
                    } else {
                        this.get_chart(rec);
                    }
                } else {
                    self.graphref.el.appendChild(this.createGraphTextElement("Select a Model first."));
                }
            } else if (rec.ks_data_calculation_type === "query" && rec.ks_query_result) {
                if (rec.ks_xlabels && rec.ks_ylabels) {
                    this.get_chart(rec);
                } else {
                    self.graphref.el.appendChild(this.createGraphTextElement("Please choose the X-labels and Y-labels"));
                }
            } else {
                self.graphref.el.appendChild(this.createGraphTextElement("Please run the appropriate Query"));
            }
        }
    }
