...
class ProductMchSetter(models.Model):
    _name = 'product.mch.setter2'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = 'Gán MCH cho sản phẩm new'
    ...
    categ_id_level_1 = fields.Many2one('product.category2',
                                       string='MCH1',
                                       domain="[('parent_id', '=', False),('category_level', '=', 1)]",
                                       store=True, readonly=False, tracking=True,
                                       )
    categ_id_level_2 = fields.Many2one('product.category2',
                                       string='MCH2',
                                       domain="[('parent_id', '=?', categ_id_level_1),('category_level', '=', 2)]",
                                       store=True, readonly=False, tracking=True,
                                       )
    categ_id_level_3 = fields.Many2one('product.category2',
                                       string='MCH3',
                                       domain="[('parent_id', '=?', categ_id_level_2),('category_level', '=', 3)]",
                                       store=True, readonly=False, tracking=True,
                                       )
    categ_id_level_4 = fields.Many2one('product.category2',
                                       string='MCH4',
                                       domain="[('parent_id', '=?', categ_id_level_3),('category_level', '=', 4)]",
                                       store=True, readonly=False, tracking=True,
                                       )
    categ_id_level_5 = fields.Many2one('product.category2',
                                       string='MCH5',
    ...
    def action_processed_and_next(self):
        ...
        next_product = self.search([('user_id', '=', False), ('branch_id', '=', self.branch_id.id), ('processed', '=', False)], limit=1)
