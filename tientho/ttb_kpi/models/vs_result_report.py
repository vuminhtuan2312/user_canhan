...
class BaoCaoKetQuaChamVS(models.AbstractModel):
    _name = 'kpi.result.vs.report.handler'
    _inherit = 'account.report.custom.handler'
    _description = 'Báo cáo kết quả chấm VS'
    ...
    def _dynamic_lines_generator(self, report, options, all_column_groups_expression_totals=None, warnings=None):
        ...
        query = f"""
            SELECT
                ...
                COUNT(DISTINCT t.id) FILTER (WHERE t."group" = 'branch_mannager') - COUNT(DISTINCT t.id) FILTER (WHERE t."group" = 'branch_mannager' AND t.state = 'done') AS total_unchecked_branch,
                ...
                COALESCE((
                    COUNT(DISTINCT t.id) FILTER (WHERE t."group" = 'region_manager' AND t.state = 'done') * 1.0
                ) / NULLIF(
                    COUNT(DISTINCT t.id) FILTER (WHERE t."group" = 'region_manager'), 0
                ),0) * 100 AS done_percentage_direct,
            ...
            LEFT JOIN ttb_area c ON t.area_id = c.id
        ...
        for branch in group_branch_id:
            ...
            for item in result_branch:
                ...
                for column in options['columns']:
                    expr = column['expression_label']
                    if expr == 'total_need_check':
                        value = item.get('total_need_check_region_cross') + item.get('total_need_check_region_direct')
                    elif expr == 'total_checked':
        ...
        total_value = {'brand_name': 'Tổng',
                       ...
                       'total_unchecked_manage': t_total_unchecked_manage, 'avg_score_manage': avg_score_manage_final,
                       'total_checked_branch': t_total_checked_branch,
                       'total_unchecked_branch': t_total_unchecked_branch, 'avg_score_branch': avg_score_branch_final,
                       ...
                       'total_overdue_manage': t_total_overdue_manage, 'total_overdue_branch': t_total_overdue_branch,
